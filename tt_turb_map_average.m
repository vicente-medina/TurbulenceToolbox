...
function ttt_turb_map_average(VEL_OUT)
  ...
  [~,pulse,velocities_pulse]=ttt_burst_calc_aux(VEL_OUT);
   ...
   %v_avg_pul_dur=velocities_pulse.average_struct_dur;
   %u=v_avg_pul_dur.D4(:,1);
   %v=v_avg_pul_dur.D4(:,2);
   %w=v_avg_pul_dur.D4(:,3);
   ...
   %u=v_avg_pul_dur.D10(:,1);
   %v=v_avg_pul_dur.D10(:,2);
   %w=v_avg_pul_dur.D10(:,3);
   %plot(u,w,'m.',[min(w) max(w)],[0 0],'k-',[0 0],[min(w) max(w)],'k-','MarkerSize',5);
   %And so on
   ...
   % the next function.
   [uc,co,nd,dt]=ttt_range_pulse_duration(velocities_pulse.average,pulse.duration);
